...
// -Le programme ajoute des fonctions pour créer des croisements dans le labyrinthe
// -La fonction de recherche de chemin est modifiée. Le principe retenu est celui 
...
#define PASSAGE		5	// passage pour créer des croisements entre voies séparées
...
// Créer des passages afin de doter le labyrinthe de ciruits.
// Il s'agit simpelent de creuser du mur qui sépare deux chemins.
// Le nombre de passages créés est aléatoire de façon un peu statistique.
void CreatePassages()
{
...
// Deux chemins peuvent être reliés à certaines conditions :
// -la position concernée pour faire passage doit être du mur
...
// est de procéder comme le petit Poucet : en semant des cailloux qui 
// signalent le chemin déjà parcouru. Chaque position vue prend ainsi la 
// valeur CHERCHE différente de la valeur CHEMIN. Si la recherche tombe sur
// une position à CHERCHE elle stope les appls récursifs
int Chemin(int x, int y, int olddir, int posx, int posy)
{
	int i, nx, ny;

	// 1 
	// si sortie matrice ou retombe sur chemin déjà vu
